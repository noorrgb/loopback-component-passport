...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var defaultCallback = function(req, res, next) {
    ...
    options.guestUserId = req.query.guestUserId;
            ...
            if (info && info.accessToken) {
              if (!!options.json) {
                return res.json({
                  ...
                  'facebookProfile': info.identity.profile,
                  'user': {
                    'id': user.id,
                    'firstName': info.identity.profile.name.givenName,
                    'lastName': info.identity.profile.name.familyName,
                    'email': (info.identity.profile.emails && info.identity.profile.emails[0] && info.identity.profile.emails[0].value),
                    'gender': info.identity.profile.gender,
                    'pictureUrl': (info.identity.profile.photos && info.identity.profile.photos[0] && info.identity.profile.photos[0].value)
                  }
                });
              } else {
