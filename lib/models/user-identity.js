...
module.exports = function(UserIdentity) {
  ...
  UserIdentity.login = function(provider, authScheme, profile, credentials,
    ...
    userIdentityModel.findOne({
      ...
      userObj.pictureUrl = (profile.photos && profile.photos[0] && profile.photos[0].value);
      userObj.isFacebookUser = true;
      ...
      if (options.guestUserId){
        query = {
          id: parseInt(options.guestUserId)
        };
      }
      else if (userObj.email && userObj.username) {
        query = {
          or: [{
              facebookId: userObj.username
            },
            {
              email: userObj.email
            },
      ...
      userModel.findOrCreate({
        ...
        if(user){
          var usrTmp = {};
          usrTmp.facebookId = userObj.username;
          usrTmp.username = userObj.username;
          usrTmp.firstName = profile.name.givenName;
          usrTmp.lastName = profile.name.familyName;
          usrTmp.gender = profile.gender;
          usrTmp.pictureUrl = (profile.photos && profile.photos[0] && profile.photos[0].value);
          usrTmp.isFacebookUser = true;
          usrTmp.email = (profile.emails && profile.emails[0] && profile.emails[0].value);
          usrTmp.type = '';
          user.updateAttributes(usrTmp);
        }
